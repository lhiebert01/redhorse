#!/usr/bin/env python3
"""
Fire Horse 2026 - Zodiac Badge Image Generator (Version 2)
Uses the google-genai library (same as the TypeScript code)

Usage:
    python generate_zodiac_badges_v2.py
...
def generate_image_new_sdk(client, prompt: str, element: str, animal: str) -> bytes | None:
...
def generate_image_old_sdk(model, prompt: str, element: str, animal: str) -> bytes | None:
    """Generate an image using the old google-generativeai SDK."""
    try:
        print(f"  Generating image with google-generativeai SDK...")
...
def main():
    ...
    print("=" * 60)
    print("Fire Horse 2026 - Zodiac Badge Image Generator (v2)")
    print("=" * 60)
    ...
    if USE_NEW_SDK:
        ...
        print("Initializing google-generativeai model...")
    ...
    for i, (element, animal) in enumerate([(e, a) for e in elements for a in animals], 1):
        ...
        # Generate image
        if USE_NEW_SDK:
            image_data = generate_image_new_sdk(client, prompt, element, animal)
        else:
            image_data = generate_image_old_sdk(model, prompt, element, animal)
